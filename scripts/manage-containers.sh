...
prepare_env_file()
{
    ...
    local _default_vars='${POSTGRES_USER} ${POSTGRES_PASSWORD} ${RESEND_API_KEY} ${KEYCLOAK_ACTIVE_CONTAINER} ${ANTHROPIC_AUTH_TOKEN} ${ANTHROPIC_BASE_URL} ${ANTHROPIC_API_KEY} ${KEYCLOAK_ADMIN_USER} ${KEYCLOAK_ADMIN_PASSWORD} ${TOKEN_SECRET} ${EMAIL_SERVICE_API_KEY}'
...
run_container()
{
    ...
    local container_tmpfs="${CONTAINER_TMPFS:-/tmp /app/scripts/logs /app/apps/findclass/scripts/python/cache:uid=1001,gid=1001,mode=0755 /app/apps/findclass/scripts/python/logs:uid=1001,gid=1001,mode=0755 /app/apps/findclass/api/crawl-output:uid=1001,gid=1001,mode=0755}"
