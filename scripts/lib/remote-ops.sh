...
transfer_image()
{
    ...
    # registry:3 起（2026-09-13）：v2 的 garbage-collect 不跟踪 OCI index→子 manifest
    # 引用链，--delete-untagged 会把 buildx 推送的平台 manifest 当孤儿删掉，
    # tag 全部变成「index 在但 children 404」的坏数据（2026-09-13 实证）。
    # v3 修复了 index 引用跟踪，GC 才能与 retention（keep 2）共存。
    if ! docker ps --format '{{.Names}}' | grep -q noda-registry; then
        ...
        docker run -d -p 5001:5000 --name noda-registry --restart unless-stopped \
            ...
            -v registry-data:/var/lib/registry registry:3 >/dev/null 2>&1
        sleep 2
    fi
...
#   $3: env_prefix - 可选 "VAR=val" 前缀（如 NODA_STATIC_IMAGE=noda-static:<commit>，
#       供 compose 模板 ${NODA_*_IMAGE} 插值——apps 只发 commit tag，r4s 上无 latest）
# 返回: compose 命令的退出码
remote_compose()
{
    local cmd="$1"
    ...
    remote_exec "cd /opt/noda/noda-infra && ${env_prefix} docker compose $compose_files $cmd"
}
