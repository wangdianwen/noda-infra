...
# -------------------------------------------
# Registry 定期维护 (CACHE-04)
# -------------------------------------------
# 保留策略（每仓库最新 2 版 + retired 仓库清空）+ blob GC（DELETE 只标记，
# 空间由 GC 回收；GC 需 registry 容器内执行）
...
registry_maintenance()
{
    ...
    source "$(dirname "${BASH_SOURCE[0]}")/image-cleanup.sh"
...
# 返回：无（编排 Jenkins workspace + pnpm store + npm cache + registry 清理）
