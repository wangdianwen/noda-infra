...
docker exec noda-infra-postgres-1 pg_dump -U postgres test_restore_quick \
...
# 验证恢复的数据
user_count=$(docker exec noda-infra-postgres-1 psql -U postgres -d test_restore_restored -t -c \
