...
- [x] **INFRA-01**: noda-ops 依赖审计（确认 wget/gnupg/coreutils 运行时是否必需，非必需移到构建阶段）
- [x] **INFRA-02**: backup Dockerfile 清理（移除冗余层、统一 RUN 指令、添加 .dockerignore）
...
| INFRA-01 | Phase 52 | Complete |
| INFRA-02 | Phase 52 | Complete |
