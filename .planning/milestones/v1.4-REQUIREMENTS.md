# Requirements Archive: v1.4 CI/CD 零停机部署
...
- [x] **JENK-01**: 管理员可以通过 `setup-jenkins.sh install` 在宿主机原生安装 Jenkins LTS
- [x] **JENK-02**: 管理员可以通过 `setup-jenkins.sh uninstall` 完全卸载 Jenkins 及其残留文件
- [x] **JENK-03**: Jenkins 用户自动加入 docker 组，可直接操作 Docker daemon
- [x] **JENK-04**: Jenkins 安装后首次启动可获取初始管理员密码

### Pipeline 核心

- [x] **PIPE-01**: Pipeline 按 Pre-flight -> Build -> Test -> Deploy -> Health Check -> Switch -> Verify -> Cleanup 八阶段执行
- [x] **PIPE-02**: 每次构建的镜像使用 Git SHA 短哈希标签（如 `findclass-ssr:abc1234`），替代 latest
- [x] **PIPE-03**: 构建阶段失败时自动中止 Pipeline，不进入部署阶段
- [x] **PIPE-04**: Pipeline 通过手动触发执行，不支持自动触发
- [x] **PIPE-05**: 部署失败时自动归档构建日志和容器日志到 Jenkins

### 蓝绿部署

- [x] **BLUE-01**: 同一时刻存在 blue 和 green 两个 findclass-ssr 容器实例，活跃容器对外服务，目标容器等待验证
- [x] **BLUE-02**: nginx 通过 upstream include 文件指向活跃容器，Pipeline 切换时更新文件并 `nginx -s reload`
- [x] **BLUE-03**: 活跃环境状态通过 `/opt/noda/active-env` 文件持久化追踪（内容为 `blue` 或 `green`）
- [x] **BLUE-04**: 蓝绿容器通过 `docker run` 独立管理生命周期，不通过 docker-compose.yml 管理
- [x] **BLUE-05**: 蓝绿容器均在 `noda-network` Docker 网络上，nginx 通过容器名 DNS 解析访问

### 测试与质量门禁

- [x] **TEST-01**: Pipeline Test 阶段执行 `pnpm lint`，lint 不通过则中止部署
- [x] **TEST-02**: Pipeline Test 阶段执行 `pnpm test`，单元测试不通过则中止部署
- [x] **TEST-03**: 部署后对目标容器执行 HTTP 健康检查（直接 curl 容器内部端点），最多重试 10 次每次间隔 5 秒
- [x] **TEST-04**: 流量切换后通过 nginx 执行 E2E 验证（curl 外部可达性），确认完整请求链路正常
- [x] **TEST-05**: 健康检查或 E2E 验证失败时，不切换流量、不停止旧容器，自动回滚到当前活跃环境

### 增强特性

- [x] **ENH-01**: Pipeline Pre-flight 阶段检查数据库备份是否在 12 小时内，不满足则阻止部署
- [x] **ENH-02**: 部署成功后自动调用 Cloudflare API 清除 CDN 缓存（index.html 和静态资源）
- [x] **ENH-03**: Pipeline Cleanup 阶段自动清理超过 7 天的旧 Docker 镜像，防止磁盘空间耗尽
...
|-------------|-------|--------|
| JENK-01 | Phase 19 | Complete |
| JENK-02 | Phase 19 | Complete |
| JENK-03 | Phase 19 | Complete |
| JENK-04 | Phase 19 | Complete |
| BLUE-02 | Phase 20 | Complete |
| BLUE-01 | Phase 21 | Complete |
| BLUE-03 | Phase 21 | Complete |
| BLUE-04 | Phase 21 | Complete |
| BLUE-05 | Phase 21 | Complete |
...
| ENH-03 | Phase 24 | Complete |
| ENH-04 | Phase 25 | Complete |

**Coverage:**
- v1.4 requirements: 23 total, 23 Complete
