...
# 前置：DOPPLER_TOKEN 环境变量（具有项目管理权限的 Token）
# ============================================
...
if [[ -z "${DOPPLER_TOKEN:-}" ]]; then
    ...
    log_error "使用方法: export DOPPLER_TOKEN='<personal-or-cli-token>'"
    exit 1
fi
...
其余字段保持不变（NODE_ENV, NEXT_PUBLIC_LOCAL_API_URL, KEYCLOAK_INTERNAL_URL, RESEND_API_KEY, ANTHROPIC_*, KEYCLOAK_ADMIN_URL, EMAIL_SERVICE 等）。
  ...
  <verify>
    <automated>grep -c 'DOPPLER_CONFIG' scripts/lib/secrets.sh | grep -v '^0$' && bash -n scripts/doppler-setup.sh && grep -c 'noda_preprod' docker/env-noda-apps-preprod.env | grep -v '^0$' && grep 'KEYCLOAK_REALM=noda-preprod' docker/env-noda-apps-preprod.env | grep -c 'noda-preprod'</automated>
  </verify>
  <done>
...
前置条件：需要 DOPPLER_TOKEN 具有项目管理权限（非 service token，因为需要创建 config 和 token）。使用个人 CLI token 或 admin token。
...
|----------|-------------|
| doppler-setup.sh -> Doppler API | 通过 doppler CLI 操作，需要 DOPPLER_TOKEN |
...
6. `doppler secrets get POSTGRES_DB --plain --project noda --config pre` 返回 `noda_preprod`
7. `doppler secrets get KEYCLOAK_REALM --plain --project noda --config pre` 返回 `noda-preprod`
8. `bash scripts/verify-preprod-isolation.sh` 退出码为 0
</verification>
